...
console.log("reqbody",req.body)
    if (draftId) {
      ...
      const existingDraft = await InvoiceDraft.findOne({ draftId, createdBy: userId });
      if (!existingDraft) {
    ...
    let counter = await DraftCounterModel.findOne({ year: currentYear, createdBy: userId });
    ...
    const newDraftId = `DRAFT-${currentYear}-${String(counter.seq).padStart(3, "0")}`;
...
//     // 1️⃣ Drafts fetch karo (delete hone se pehle list dikhane ke liye)
//     const drafts = await InvoiceDraft.find(filter).sort({ createdAt: -1 });

//     // 2️⃣ Sare drafts delete karo
//     await InvoiceDraft.deleteMany(filter);

//     // 3️⃣ Counter reset karo
